...
interface ListingFormData {
  country: string
  city: string
  propertyType: string // This will now hold the _id
  areaSuburb: string
  exactAddress?: string
  approxSize: string
  budgetFrom: string
...
export function ListingForm() {
  ...
  const onSubmit = (formData: ListingFormData) => {
    const payload = {
      type: formData.propertyType, // This is now the _id from API
    ...
    mutation.mutate(payload as data)
  }
      ...
      <div className="mx-auto container px-5 py-8">
        <div className="mb-8 flex items-start  justify-center text-center md:mb-12">
          <div className="">
            ...
            <p className="mt-2 text-gray-400">
              Share your property requirements with thousands of landlords
            </p>
          </div>
        </div>
        ...
        <Card className="border-none bg-white/10 p-4 md:p-8">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              {/* DYNAMIC PROPERTY TYPE FROM API */}
              <div>
                <label className="mb-3 block text-base font-medium text-white">
                ...
                {isLoadingPropertyTypes ? (
                  ...
                  <div className="h-[51px] flex items-center justify-center border border-red-500/50 rounded-md bg-red-500/10 text-red-400 text-sm">
                    Failed to load types
                  </div>
                ) : propertyTypes.length === 0 ? (
                  <div className="h-[51px] flex items-center justify-center border border-amber-500/50 rounded-md bg-amber-500/10 text-amber-400 text-sm">
              ...
              <div>
                ...
                <Input
                  {...register('areaSuburb')}
                  placeholder="e.g. CBD, Parramatta"
                  className="h-[51px] border border-[#BFBFBF] bg-transparent text-white placeholder-[#BFBFBF]"
                />
              </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <div className="flex flex-col">
                <label className="mb-3 text-base font-medium text-white">
                  Location on Map {selectedLocation && '(Selected)'}
                </label>
                <Button
                  type="button"
                  variant="outline"
                  className="h-[51px] bg-gradient hover:bg-gradient/80 hover:text-white text-white border-0"
              ...
              <div>
                ...
                <Input
                  {...register('approxSize', { required: 'Size is required' })}
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div>
                ...
                <div className="flex gap-3 items-center">
                  <Input
                    {...register('budgetFrom', { required: 'Budget from is required' })}
                    type="number"
                    placeholder="$ From"
                    className="h-[51px] border border-[#BFBFBF] bg-transparent text-white"
                  />
                  <span className="text-white">to</span>
                  <Input
                    {...register('budgetTo', { required: 'Budget to is required' })}
                    type="number"
                    placeholder="$ To"
                    className="h-[51px] border border-[#BFBFBF] bg-transparent text-white"
                  />
                ...
                {(errors.budgetFrom || errors.budgetTo) && (
                  <p className="mt-1 text-sm text-red-400">Both budget fields are required</p>
                )}
              </div>
              ...
              <div>
                ...
                <Input
                  {...register('requiredFromDate', { required: 'Date is required' })}
                  type="date"
                  className="h-[51px] border border-[#BFBFBF] bg-transparent text-white"
                />
            ...
            <div>
              ...
              <Textarea
                ...
                rows={6}
                placeholder="Tell landlords what you're looking for..."
                className="border border-[#BFBFBF] bg-transparent text-white placeholder-[#BFBFBF] p-4"
              />
            </div>
            ...
            <Button
              type="submit"
              disabled={mutation.isPending || isLoadingPropertyTypes}
              className="w-full h-[56px] text-lg font-semibold bg-gradient hover:bg-gradient/80 text-white rounded-xl flex items-center justify-center gap-2"
            >
              {mutation.isPending ? (
                <>
